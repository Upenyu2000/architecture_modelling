...
def submit(job: Job, task: Callable[[Callable[[int, str], None]], tuple[Path, str]]) -> Job:
    def runner() -> None:
        update_job(job.id, 1, "Starting", status="running")
        try:
            output_path, output_url = task(lambda progress, message: update_job(job.id, progress, message))
            update_job(job.id, 100, "Complete", status="completed", output_path=str(output_path), output_url=output_url)

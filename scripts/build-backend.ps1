    ...
    for ($Attempt = 0; $Attempt -lt 60; $Attempt++) {
        ...
        try {
            ...
            if ($Response.StatusCode -eq 200) {
                ...
                if ($Payload.version -ne "1.5.5") {
                    throw "Packaged backend reported version $($Payload.version), expected 1.5.5."
                }
                $Healthy = $true
                break
            }
...
Remove-Item -Recurse -Force $TestData -ErrorAction SilentlyContinue
Remove-Item -Force $StdOut, $StdErr -ErrorAction SilentlyContinue
Write-Host "Packaged backend health check passed with version 1.5.5."

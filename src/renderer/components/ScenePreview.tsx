...
function boundaryDistance(point: Point, polygon: Point[]): number {
  const p = new THREE.Vector2(point[0], point[1]);
  let minimum = Number.POSITIVE_INFINITY;
  polygon.forEach((start, index) => {
    const end = polygon[(index + 1) % polygon.length];
    minimum = Math.min(minimum, distanceToSegment(p, start, end));
  });
  return minimum;
}

function roomsForOpening(scene: SceneManifest, opening: Opening): RoomShape[] {
  const wall = opening.wall_id ? scene.walls.find((candidate) => candidate.id === opening.wall_id) : null;
...
function Wall({ wall, scene, cutaway }: { wall: WallSegment; scene: SceneManifest; cutaway: boolean }) {
  ...
  return (
    <group>
      {bodyIntervals.map(([start, end], index) => <WallBox key={`body-${index}`} wall={wall} startOffset={start} endOffset={end} bottom={0} height={visibleHeight} spec={spec} />)}
      {projected.map(({ opening, start, end }) => {
        ...
        return visibleHeight > top ? <WallBox key={opening.id} wall={wall} startOffset={start} endOffset={end} bottom={top} height={visibleHeight - top} spec={spec} /> : null;
      })}
    </group>
  );
}
    ...
    <group position={[x, sill, z]} rotation={[0, rotation, 0]}>
      ...
      {bay ? [-1, 1].map((sign) => (
        <mesh key={sign} position={[sign * opening.width * 0.42, height / 2, 0.12]} rotation={[0, -sign * Math.PI / 7, 0]}>
          <boxGeometry args={[opening.width * 0.24, height, 0.025]} /><meshPhysicalMaterial color="#72b7df" transparent opacity={0.3} />
        </mesh>
      )) : null}
    </group>
  );
...
function DoorFrame({ opening, scene }: { opening: Opening; scene: SceneManifest }) {
  ...
  const depth = Math.max(0.12, scene.walls.find((wall) => wall.id === opening.wall_id)?.thickness ?? 0.16) + 0.035;
  return (
    <group position={[x, 0, z]} rotation={[0, rotation, 0]}>
      {[-1, 1].map((sign) => <mesh key={sign} position={[sign * (opening.width / 2 + frame / 2), height / 2, 0]} castShadow><boxGeometry args={[frame, height, depth]} /><meshStandardMaterial color="#5F4430" roughness={0.42} /></mesh>)}
      <mesh position={[0, height + frame / 2, 0]} castShadow><boxGeometry args={[opening.width + frame * 2, frame, depth]} /><meshStandardMaterial color="#5F4430" roughness={0.42} /></mesh>
      <mesh position={[0, 0.025, 0]}><boxGeometry args={[opening.width + frame * 2, 0.05, depth]} /><meshStandardMaterial color="#77563D" roughness={0.5} /></mesh>
    </group>
  );
}

function InteractiveDoor({ opening, scene, isOpen, onToggle }: { opening: Opening; scene: SceneManifest; isOpen: boolean; onToggle: () => void }) {
  ...
  const click = (event: { stopPropagation: () => void }) => { event.stopPropagation(); if (opening.interactive !== false) onToggle(); };
  let leaves: React.ReactNode;

  if (opening.opening_type === 'revolving_door') {
    leaves = <><mesh position={[0, height / 2, 0]}><cylinderGeometry args={[width / 2, width / 2, height, 32, 1, true]} /><meshPhysicalMaterial color="#91c4db" transparent opacity={0.18} side={THREE.DoubleSide} /></mesh><group ref={revolving} position={[0, height / 2, 0]}><mesh>{material}<boxGeometry args={[width, height, 0.045]} /></mesh><mesh rotation={[0, Math.PI / 2, 0]}>{material}<boxGeometry args={[width, height, 0.045]} /></mesh></group></>;
    ...
    leaves = <group ref={slider}><mesh position={[double ? -width * 0.25 : 0, height / 2, 0]} castShadow><boxGeometry args={[double ? width / 2 : width, height, 0.045]} />{opening.opening_type === 'sliding_glass_door' ? <meshPhysicalMaterial color="#72b7df" transparent opacity={0.35} transmission={0.6} /> : material}</mesh>{double ? <mesh position={[width * 0.25, height / 2, 0.04]} castShadow><boxGeometry args={[width / 2, height, 0.045]} />{material}</mesh> : null}</group>;
  } else {
    ...
    leaves = double ? <><group ref={left} position={[-width / 2, 0, 0]}><mesh position={[leafWidth / 2, height / 2, 0]} castShadow><boxGeometry args={[leafWidth, height, 0.045]} />{material}</mesh></group><group ref={right} position={[width / 2, 0, 0]}><mesh position={[-leafWidth / 2, height / 2, 0]} castShadow><boxGeometry args={[leafWidth, height, 0.045]} />{material}</mesh></group></> : <group ref={left} position={[rightHinged ? width / 2 : -width / 2, 0, 0]}><mesh position={[rightHinged ? -width / 2 : width / 2, height / 2, 0]} castShadow><boxGeometry args={[width, height, 0.045]} />{material}</mesh></group>;
  }

  return <><DoorFrame opening={opening} scene={scene} /><group position={[x, 0, z]} rotation={[0, rotation, 0]} onClick={click}>{leaves}</group></>;
}
...
function pointInsidePassage(point: THREE.Vector2, wall: WallSegment, openings: Opening[], openDoorIds: Set<string>): boolean {
  return openingsForWall(wall, openings).some(({ opening, centre }) => {
    ...
    return Math.abs(along - centre) <= opening.width / 2 + 0.12;
  });
}

function FirstPersonRig({ scene, openDoorIds, onToggleDoor, onRoomChange }: {
  scene: SceneManifest;
  ...
  useEffect(() => {
    ...
    onRoomChange(roomAt(scene, [start[0], start[2]])?.id ?? null);
    const down = (event: KeyboardEvent) => {
      ...
      forward.y = 0;
      forward.normalize();
      const candidate = scene.openings
      ...
      if (candidate) onToggleDoor(candidate.opening.id);
    };
    const up = (event: KeyboardEvent) => keys.current.delete(event.code);
    window.addEventListener('keydown', down);
    window.addEventListener('keyup', up);
    return () => { window.removeEventListener('keydown', down); window.removeEventListener('keyup', up); };
  }, [camera, scene, onRoomChange, onToggleDoor, start[0], start[1], start[2]]);
  ...
  useFrame((_, delta) => {
    ...
    const blocked = scene.walls.some((wall) => distanceToSegment(point, wall.start, wall.end) < wall.thickness / 2 + 0.23 && !pointInsidePassage(point, wall, scene.openings, openDoorIds));
    if (!blocked) {
      ...
      onRoomChange(roomAt(scene, [proposed.x, proposed.z])?.id ?? null);
    } else velocity.current.multiplyScalar(0.1);
  });
...
function SceneContent({ project, scene, referenceUrl, view }: { project: Project; scene: SceneManifest; referenceUrl?: string; view: RenderedViewMode }) {
  const centreX = scene.width_m / 2;
  const centreZ = scene.depth_m / 2;
  const largest = Math.max(scene.width_m, scene.depth_m, 4);
  const cutaway = view === 'isometric';
  const [currentRoomId, setCurrentRoomId] = useState<string | null>(null);
  const [openDoorIds, setOpenDoorIds] = useState<Set<string>>(() => new Set(scene.openings.filter((opening) => opening.default_open || opening.opening_type === 'open_passage').map((opening) => opening.id)));

  useEffect(() => {
    setOpenDoorIds((current) => {
      const next = new Set([...current].filter((id) => scene.openings.some((opening) => opening.id === id)));
      scene.openings.forEach((opening) => { if (opening.default_open || opening.opening_type === 'open_passage') next.add(opening.id); });
      return next;
    });
  }, [scene.openings]);

  const visibleRooms = useMemo(() => view === 'walkthrough' ? connectedVisibleRooms(scene, currentRoomId, openDoorIds) : new Set(scene.rooms.map((room) => room.id)), [scene, currentRoomId, openDoorIds, view]);
  const toggleDoor = (openingId: string) => setOpenDoorIds((current) => { const next = new Set(current); if (next.has(openingId)) next.delete(openingId); else next.add(openingId); return next; });
  const assetTexture = (assetId: string, objectType: string): string | undefined => {
    ...
    const matching = Object.entries(project.assets).find(([key]) => key.endsWith(`/${objectType}`) || key.endsWith(`/${objectType === 'sofa' ? 'couch' : objectType}`));
  ...
  return (
    <>
      ...
      {scene.openings.map((opening) => isWindow(opening) ? <WindowMarker key={opening.id} opening={opening} scene={scene} /> : <InteractiveDoor key={opening.id} opening={opening} scene={scene} isOpen={openDoorIds.has(opening.id)} onToggle={() => toggleDoor(opening.id)} />)}
      ...
      {scene.assets.filter((asset) => !occupiedTypes.has(asset.slot === 'couch' ? 'sofa' : asset.slot)).map((asset) => <FurnitureModel key={asset.id} id={asset.id} objectType={asset.slot} position={asset.position} size={asset.size} rotationDeg={THREE.MathUtils.radToDeg(asset.rotation_y)} style={{ style: 'modern', material: asset.slot.includes('fridge') ? 'painted_metal' : 'fabric', color: scene.materials.accent.hex_color }} textureUrl={absoluteUrl(asset.source_url)} visible={!asset.room_id || visibleRooms.has(asset.room_id)} />)}
      ...
      {view === 'top' ? <><ResponsiveTopCamera scene={scene} /><OrbitControls makeDefault target={[centreX, 0, centreZ]} enableRotate={false} enableDamping /></> : view === 'isometric' ? <><ResponsiveIsometricCamera scene={scene} /><OrbitControls makeDefault target={[centreX, 0.7, centreZ]} enableDamping /></> : <><PerspectiveCamera makeDefault position={scene.first_person_start ?? [centreX, 1.7, centreZ]} fov={72} near={0.04} far={largest * 12} /><FirstPersonRig scene={scene} openDoorIds={openDoorIds} onToggleDoor={toggleDoor} onRoomChange={setCurrentRoomId} /></>}
    </>
  );
}
...
export function ScenePreview({
  ...
  useEffect(() => { if (scene?.layout_mode === 'manual' && scene.rooms.length === 0) setView('edit'); }, [scene?.layout_mode, scene?.rooms.length]);
  const renderedView: RenderedViewMode = view === 'top' || view === 'walkthrough' ? view : 'isometric';

  return (
    <section className="viewer-panel">
      <div className="viewer-header">
        <div><span className="eyebrow">4. Synchronized viewports</span><h2>{scene ? `${scene.rooms.length} rooms · ${scene.walls.length} walls · ${scene.openings.length} openings · ${scene.fixtures_and_furniture.length} interiors` : 'Waiting for a layout'}</h2></div>
      ...
      <div className={view === 'edit' || view === 'openings' || view === 'interior' ? 'canvas-wrap editor-canvas' : 'canvas-wrap'}>
        {scene && project ? (
          view === 'structure' && structureUrl ? <div className="structure-preview"><img src={structureUrl} alt="Detected walls, rooms, doors, windows and furniture" /><div><span className="green-key" /> Model/vector structure <span className="orange-key" /> Room boundaries</div></div>
            : view === 'edit' ? <RoomLayoutEditor scene={scene} referenceUrl={referenceUrl} busy={busy} onAddRoom={onAddRoom} onUpdateRoom={onUpdateRoom} onDeleteRoom={onDeleteRoom} onRenameRoom={onRenameRoom} />
              : view === 'openings' ? <OpeningEditor scene={scene} referenceUrl={referenceUrl} busy={busy} onAddOpening={onAddOpening} onUpdateOpening={onUpdateOpening} onDeleteOpening={onDeleteOpening} />
                : view === 'interior' ? <InteriorDesignEditor project={project} scene={scene} busy={busy} onAddFurniture={onAddFurniture} onUpdateFurniture={onUpdateFurniture} onDeleteFurniture={onDeleteFurniture} />
                  : view === 'data' ? <DataSummary scene={scene} />
                    : <div className="three-view-wrap"><Canvas shadows dpr={[1, 2]} gl={{ preserveDrawingBuffer: true, antialias: true }}><color attach="background" args={[view === 'walkthrough' ? '#dce8ef' : '#0a1711']} />{view === 'isometric' ? <fog attach="fog" args={['#0a1711', 35, 120]} /> : null}<Suspense fallback={null}><SceneContent project={project} scene={scene} referenceUrl={referenceUrl} view={renderedView} /></Suspense></Canvas>{view === 'walkthrough' ? <div className="walkthrough-help"><strong>Click inside to look around</strong><span>WASD / arrows move · Shift runs · E or click opens doors · closed portals cull adjoining rooms · Esc releases mouse</span></div> : null}</div>
